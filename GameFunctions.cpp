...
// xxxxxxxxxxxxx
// -0x13
RelocAddr<TESObjectREFR_GetInventoryItemCount_t*> TESObjectREFR_GetInventoryItemCount (0x28E440);
...
RelocAddr<TESObjectREFR_LookupRefByHandle_t*>     TESObjectREFR_LookupRefByHandle   (0x002132E0);

// \x48\x0F\x44\xD6\x48\x85\xD2\x0F\x85\x00\x00\x00\x00\x48\x85\xC0\x0F\x85
// xxxxxxxxx????xxxxx
// -0x2E
RelocAddr<TESForm_GetOwner_t*> TESForm_GetOwner (0x2A6860);

// 48 85 C9 0F 84 C1 00 00 00 0F B6 41 1A 83 C0 E9 83 F8 1D 0F 87 B1 00 00 00 -0x16
RelocAddr<TESForm_GetWeight_t*>  TESForm_GetWeight (0x1A1920);

// 48 85 D2 74 2F 8B 4B 0C FF C9 8B 44 24 48 48 23 C8 48 8D 04 49 48 8D 04 C2 48 83 78 10 00 74 14 -0x4C
RelocAddr<TESForm_LookupFormByID_t*>   TESForm_LookupFormByID (0x194420);
...
*/
RelocAddr<ECCData_ctor_t*>  ECCData_ctor (0x1D93F0);
RelocAddr<ECCData_dtor_t*>  ECCData_dtor (0x1D94E0);

// 33 FF 44 8B E7 48 8B 49 08 0F B7 D7 66 89 55 40 48 85 C9 74 15 8B C7 80 79 1A 3E -0x32
RelocAddr<ECCData_InitContainer_t*> ECCData_InitContainer (0x1E9F80);
...
// first entry
RelocAddr<BaseExtraList_SetInventoryChanges_t*> BaseExtraList_SetInventoryChanges (0x10F7B0);

// 48 85 FF 74 06 0F B7 47 10 EB 05 B8 01 00 00 00 48 8B 5C 24 48 48 83 C4 30 5F C3 -0x47
RelocAddr<BaseExtraList_GetItemCount_t*>	    BaseExtraList_GetItemCount (0x1139A0);

// 48 8B 59 08 48 85 DB 74 22 48 8B 1B 48 85 DB 74 1A 48 8B CB (first match)
RelocAddr<InventoryEntryData_GetOwner_t*>		 InventoryEntryData_GetOwner (0x1D6810);

// 48 8B F0 48 85 C0 75 04 48 8B 73 40 33 C0 80 7E 1A 2B 48 0F 44 C6 48 3B EE 0F 84 80 00 00 00 80 7D 1A 0B -0x4D
RelocAddr<InventoryEntryData_IsOwnedBy_t*>		InventoryEntryData_IsOwnedBy (0x1D7780);

// 48 8B 59 08 40 32 FF 48 85 DB 74 4A 48 89 74 24 30 0F 1F 44 00 00 48 83 7B 08 00 75 06 -0xA
RelocAddr<InventoryEntryData_IsQuestItem_t*>    InventoryEntryData_IsQuestItem (0x1D6D90);

// 49 89 5B 10 49 89 73 18 33 C0 49 89 43 D8 -0x10
RelocAddr<MagicItem_GetCostliestEffectItem_t*>	 MagicItem_GetCostliestEffectItem (0x101EB0);